...
test.binomial.gam.mojo <-
  function() {
    ...
    e <- tryCatch({
      ...
      modelAndDir<-buildModelSaveMojoGAM(params_prob_data$params) # build the model and save mojo
...
setParmsData <- function(numTest=1000) {
  ...
  browser()
  train_numeric <- random_dataset_real_only(h2o.nrow(training_file), 1)
  training_file <- h2o.cbind(training_file, train_numeric)  # make sure we have at least one numeric column
