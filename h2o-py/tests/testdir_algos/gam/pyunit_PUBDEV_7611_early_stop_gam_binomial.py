...
def test_gam_model_predict():
    ...
    numIter = len(h2o_model_no_early_stop._model_json["output"]["glm_scoring_history"].cell_values)
    for ind in range(len(early_stop_metrics)):
        ...
        metric_list1 = \
            pyunit_utils.extract_field_from_twoDimTable(
                h2o_model._model_json["output"]["glm_scoring_history_early_stop"].col_header,
                h2o_model._model_json["output"]["glm_scoring_history_early_stop"].cell_values,
    ...
    print("Check if lambda_search=True, no early_stop should happen.")
    h2o_model = H2OGeneralizedAdditiveEstimator(family='binomial', gam_columns=["C11"], scale = [0.0001],
                                                stopping_rounds=max_stopping_rounds,score_each_iteration=True,
                                                stopping_metric=early_stop_metrics[ind],
    ...
    assert h2o_model._model_json["output"]["glm_scoring_history_early_stop"] == None, \
        "Early stopping should be disabled but is not."   
